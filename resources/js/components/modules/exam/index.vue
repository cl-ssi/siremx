                  ...
                  <table
                    ...
                    <tbody class="small">
                      <tr v-for="(item, index) in listarUsuariosPaginated" :key="index">
                        <td>
                          <button
                            v-if="item.date_exam_reception && item.exam_type && (item.birards_proyeccion || item.birards_ecografia || item.birards_mamografia)"
                        ...
                        <td>{{ item.id }}</td>
                        <td>{{ item.commune }}</td>
                        <td class=" text-wrap">{{ item.establishment_origin }}</td>
                        <td>{{ $moment(item.date_exam).format("DD-MM-YYYY") }}</td>
                        <td>{{ item.exam_type }}</td>
                        <td>{{ item.run + '-' + item.dv }}</td>
                        <td>{{ item.fathers_family + ' ' + item.mothers_family }}</td>
                        <td>{{ item.name }}</td>
                        <td>{{ $moment(item.birthday).format("DD-MM-YYYY") }}</td>
                        <td>{{ item.firstname + ' ' + item.lastname }}</td>
                        <td class="text-right">
                          <template v-if="listRolePermissionsByUser.includes('exam.view')">
                            <router-link class="btn btn-xs btn-default"
                              :to="{ name: 'exam.view', params: { id: item.patient_id } }">
                          ...
                          <template v-if="listRolePermissionsByUser.includes('exam.edit')">
                            <router-link class="btn btb-flat btn-xs btn-default"
                              :to="{ name: 'exam.edit', params: { id: item.id } }">
                              <i class="fas fa-pencil-alt"></i>
                            </router-link>
                          </template>
                          <template v-if="listRolePermissionsByUser.includes('exam.delete')">
                            <button v-if="item.user_id == authUser_id" class="btn btb-flat btn-xs btn-default"
